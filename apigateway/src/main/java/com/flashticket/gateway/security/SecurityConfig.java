...
/**
    ...
    Vì:
        ...
        WebFlux security dùng WebFilter, SecurityWebFilterChain, @EnableWebFluxSecurity
        Gateway chạy trên WebFlux → nên security cũng phải reactive.
        cố tình thay thế bằng MVC Security, sẽ bị lỗi nghiêm trọng và ứng dụng API Gateway sẽ không thể chạy được.
 */
@Configuration
...
public class SecurityConfig {
    ...
    public SecurityWebFilterChain securityWebFilterChain(ServerHttpSecurity http) {
        return http
                 ...
                 * Tác dụng chính của nó là chuyên tiếp nhận, kiểm tra và tự đông bắt và giải mã Access Token. 
                 * tự động chui vào Header của HTTP Request gửi đến Gateway để tìm kiếm chuỗi Authorization: Bearer <chuỗi_jwt>
                 * Từ khóa oauth2 đầu tiên đại diện cho đối tượng cấu hình của OAuth2 Resource Server.
                 * .jwt(...) nói cho Spring Security  biết Ứng dụng này là OAuth2 Resource Server và sẽ xác thực request bằng JWT,
                 * biến jwt ở đây đại diện cho đối tượng cấu hình chuyên biệt của JWT.
                 * "Sau khi nhận được JWT, thay vì dùng cách đọc quyền (roles) mặc định của Spring, hãy dùng cái Converter mà tôi tự viết (jwtAuthenticationConverter()) để lôi quyền từ trong mảng realm_access của Keycloak ra.
                 */
                .oauth2ResourceServer(oauth2 -> oauth2
                        .jwt(jwt -> jwt.jwtAuthenticationConverter(jwtAuthenticationConverter()))
                )
//                .oauth2ResourceServer(oauth2 -> oauth2
//                        .jwt(cauhinhJwt -> cauhinhJwt.jwtAuthenticationConverter( thietLapQuyenTuKeycloak() ))
//                )
                .build();
    }
