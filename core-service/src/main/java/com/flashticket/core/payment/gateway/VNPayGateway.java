...
public class VNPayGateway implements PaymentGateway {
    ...
    private static final String HMAC_SHA512 = "HmacSHA512"; // VNPay API phiên bản mới nhất (2.1.0) ép buộc phải dùng
                                                            // thuật toán HmacSHA512 mã hóa (trước kia dùng SHA256). Nếu
                                                            // dùng thuật toán khác, chữ ký sẽ sai và VNPay từ chối
                                                            // thẳng.
    private static final DateTimeFormatter VNP_DATE_FORMAT = DateTimeFormatter.ofPattern("yyyyMMddHHmmss"); // Server
                                                                                                            // của VNPay
                                                                                                            // chỉ hiểu
                                                                                                            // đúng định
                                                                                                            // dạng
                                                                                                            ...
                                                                                                            // sẽ báo
                                                                                                            // lỗi ngay.
    private static final ZoneId VN_ZONE = ZoneId.of("Asia/Ho_Chi_Minh"); // Nếu server thuê ở Mỹ (AWS, Google Cloud) thì
                                                                         // giờ hệ thống mặc định sẽ là giờ Mỹ. Nếu bạn
                                                                         // không ép nó về múi giờ Việt Nam (GMT+7),
                                                                         // VNPay sẽ thấy thời gian tạo đơn hàng bị lệch
                                                                         // mười mấy tiếng và báo lỗi "Giao dịch đã hết
                                                                         // hạn" ngay lúc vừa tạo.
    private static final int EXPIRE_MINUTES = 20; // VNPay sẽ tự hết hạn giao dịch nếu quá thời gian này và khách không
                                                  // quét thanh toán
    ...
    public String createPaymentUrl(PaymentRequest request, String ipAddress) {
        ...
        // Build params map — dùng TreeMap để tự sort theo key, VNPay bắt buộc phải sắp
        // xếp theo bảng chữ cái để làm chuẩn cho hash data và signature
        ...
        params.put("vnp_ReturnUrl", properties.getReturnUrl()); // Khi khách hàng thanh toán xong (dù thành công, thất
                                                                // bại hay bấm nút Hủy). VNPay sẽ dùng cái vnp_ReturnUrl
                                                                // này để điều hướng (redirect) trình duyệt của khách
                                                                // hàng quay trở lại website hoặc ứng dụng của mình.
     ...
     * 
     * Hiện tại đang expect frontend polling 3 giây 1 lần (tối đa 20 lần retry)
     * check status PENDING (chờ IPN update SUCCESS or FAILED)
     ...
     * <p>
     * VNPay trả về hơn 20 params với tên viết tắt khó hiểu (vnp_TxnRef,
     * vnp_TransactionNo,
     * vnp_ResponseCode...). Hàm này extract chỉ những field cần thiết và đóng gói
     * vào
     * {@link PaymentResult} — một cấu trúc chung mà PaymentService hiểu được.
     * <p>
     * <b>Chỉ gọi SAU khi verifyCallback() trả true.</b> Nếu gọi trước → có thể
     * parse
     * data giả mạo → logic sai.
     * 
     * Mapping VNPay → PaymentResult:
     * vnp_TxnRef → transactionNumber (mã giao dịch hệ thống, do ta tạo lúc
     * initiatePayment)
     * vnp_TransactionNo → providerTransactionId (mã giao dịch VNPay tạo, ví dụ:
     * "14422639")
     ...
     * 
     * Lưu ý vnp_Amount: VNPay nhân 100 để loại phần thập phân (150,000 VNĐ →
     * "15000000").
     ...
     * VNPay yêu cầu encode value trước khi hash (cùng chuẩn với query string).
     */
    ...
    private String hmacSHA512(String secret, String data) {
        try {
            ...
            // data.getBytes(StandardCharsets.UTF_8); dừng ở đây thì đúng là chuyển ký tự →
            // số ASCII (UTF-8).
            ...
            // x: Biến tất cả các chữ cái (A-F) thành chữ cái viết thường (a-f). VNPay yêu
            // cầu chữ ký phải là chữ viết thường
